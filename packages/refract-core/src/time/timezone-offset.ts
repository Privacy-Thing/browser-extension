...
const getTimeZoneFormatter = (timeZone: string): Intl.DateTimeFormat => {
   ...
   */
  const formatter = new NativeDateTimeFormat("en-CA", {
    calendar: "iso8601",
    hour: "2-digit",
    hour12: false,
    minute: "2-digit",
...
export const getTimeZoneOffsetMinutes = (timeZone: string, epochMs: number): number => {
  ...
  const instant = new NativeDate(epochMs);
  ...
  const utcForZonedParts = NativeDate.UTC(year, month - 1, day, hour, minute, second);
