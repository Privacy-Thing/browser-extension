...
const buildAllowedRetiredLines = () => {
  ...
  const exactLines = new Map([
    ...
    [
      "SECURITY.md",
      new Set([
        `on this repository. If that is unavailable to you, write to \`${stableId}\`.`,
      ]),
    ],
    [
      "CODE_OF_CONDUCT.md",
      new Set([
        `the maintainers at \`${stableId}\`. All complaints will be reviewed and`,
      ]),
    ],
    [
  ...
  if (!readRepositoryFile("SECURITY.md").includes(`\`${stableId}\``)) {
    addFinding(
      "error",
      "retired-name-allowlist",
      "SECURITY.md contact must match the stable Firefox extension identity",
    );
  }
