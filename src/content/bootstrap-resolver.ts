import type { FxUserScriptsReady } from "@privacy-brand/refract-browser/firefox";
...
export const queryUserScriptsStatus = async (): Promise<FxUserScriptsReady> => {
  const response = await safeSendForResponse<FxUserScriptsResponse>({
    type: EXTENSION_COMMAND_TYPES.getUserScriptsStatus,
  });
  return (
    response?.readiness ?? {
      hasPermission: false,
...
/**
 * Chromium async fallback: tries the chrome.storage.session preload first
 * (fastest, seeded by background on webNavigation.onBeforeNavigate), then
 * falls back to an on-demand background message.
 *
 * No timeout is applied here — the caller (Chromium adapter) wraps this
 * function in its own bounded `runtimeMessageTimeoutMs` race.
 */
